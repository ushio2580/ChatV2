...
interface QuickUserStatsProps {
  users: User[];
  theme: 'light' | 'dark';
  onClick?: () => void;
}

const QuickUserStats: React.FC<QuickUserStatsProps> = ({ 
  users, 
  theme, 
  onClick 
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="flex flex-col items-center">
          <span className={`text-lg font-bold ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
            {offlineUsers.length}
          </span>
